...
- Still needs hardening before merge/release:
  ...
  - docs index now links the v1 workflow and SCC design docs; CLI task examples, chunk-executor public symbols, transport convention options, docs-index entries, parser task choices, and public export drift checks are complete for the current checkpoint and must be rerun after any further EPC API or CLI changes。
  ...
  - full repo test pass has been completed for the current checkpoint after the latest EPC hardening commits: `uv run pytest ./dptb/tests/ -q` -> `1009 passed, 32 skipped, 12 warnings`; rerun before final merge after any further EPC changes。
- Still design-only:
...
- Recent hardening checkpoints have validated required artifact files, strict mesh chunk-size typing, SCC task gating, default minimal-fixture analysis coverage, chunk-executor and transport-convention workflow docs/export drift, SCC design gate docs, core NPZ loader diagnostics, optional path-segment safe loading, CLI NPY/NPZ/JSON input diagnostics, artifact weights diagnostics, artifact manifest object/spec guards, velocity-source aliases, and linewidth scan convention aliases.
...
- EPC default production code now has a static guard preventing required `mpi4py`, CUDA, or CuPy dependencies in v1.
