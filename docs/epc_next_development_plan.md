...
- Still needs hardening before merge/release:
  ...
  - artifact metadata validation now covers weights metadata JSON, weights shape/finite/non-negative/positive-sum checks, missing chunk/weights files, unsafe artifact filenames, fixed-vs-recomputed linewidth scan convention guards, missing required array diagnostics for persistent EPC NPZ loaders, path-axis scalar-string validation, representative metadata JSON scalar/object validation, representative object-array rejection under pickle-free loading, CLI array-loader missing-field diagnostics, summary-loader metadata/schema rejection, strict `EPCMeshSpec` chunk-size type validation, and direct k/q chunk-spec field validation; continue strict NPZ loader audit for remaining edge cases。
  - full repo test pass has been completed for the current checkpoint after the path-axis, artifact-filename, and chunk-spec hardening commits: `uv run pytest ./dptb/tests/ -q` -> `889 passed, 32 skipped, 12 warnings`; rerun before final merge after any further EPC changes。
- Still design-only:
