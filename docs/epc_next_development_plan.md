...
- Still needs hardening before merge/release:
  ...
  - full repo test pass has been completed for the current checkpoint after the docs-index drift guard commit: `uv run pytest ./dptb/tests/ -q` -> `919 passed, 32 skipped, 12 warnings`; rerun before final merge after any further EPC changes。
- Still design-only:
