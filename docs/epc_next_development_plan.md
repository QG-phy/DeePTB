...
- Still needs hardening before merge/release:
  ...
  - full repo test pass has been completed for the current checkpoint after the summary metadata diagnostic hardening commit: `uv run pytest ./dptb/tests/ -q` -> `918 passed, 32 skipped, 12 warnings`; rerun before final merge after any further EPC changes。
- Still design-only:
