...
- Still needs hardening before merge/release:
  ...
  - artifact metadata validation now covers weights metadata JSON, weights shape/finite/non-negative/positive-sum checks, missing chunk/weights files, fixed-vs-recomputed linewidth scan convention guards, missing required array diagnostics for persistent EPC NPZ loaders, CLI array-loader missing-field diagnostics, summary-loader metadata/schema rejection, and strict `EPCMeshSpec` chunk-size type validation; continue strict NPZ loader audit for remaining edge cases。
